...
-- Add category to existing installs that already have the items table
alter table public.items
  add column if not exists category text;

-- Add brand to existing installs
alter table public.items
  add column if not exists brand text;

-- Add notes to existing installs
alter table public.items
...
drop policy if exists "Only management can insert locations" on public.locations;
create policy "Only management can insert locations"
  on public.locations for insert
  with check ((select (auth.jwt()->>'user_role')) = 'management');

drop policy if exists "Only management can update locations" on public.locations;
create policy "Only management can update locations"
  on public.locations for update
  using ((select (auth.jwt()->>'user_role')) = 'management')
  with check ((select (auth.jwt()->>'user_role')) = 'management');

drop policy if exists "Only management can delete locations" on public.locations;
...
drop policy if exists "Only management can insert items" on public.items;
create policy "Only management can insert items"
  on public.items for insert
  with check ((select (auth.jwt()->>'user_role')) = 'management');

drop policy if exists "Only management can update items" on public.items;
create policy "Only management can update items"
  on public.items for update
  using ((select (auth.jwt()->>'user_role')) = 'management')
  with check ((select (auth.jwt()->>'user_role')) = 'management');

drop policy if exists "Only management can delete items" on public.items;
...
drop policy if exists "Everyone can view par levels" on public.par_levels;
create policy "Everyone can view par levels"
  on public.par_levels for select
  using (true);

drop policy if exists "Only management can insert par levels" on public.par_levels;
create policy "Only management can insert par levels"
  on public.par_levels for insert
  with check ((select (auth.jwt()->>'user_role')) = 'management');

drop policy if exists "Only management can update par levels" on public.par_levels;
create policy "Only management can update par levels"
  on public.par_levels for update
  using ((select (auth.jwt()->>'user_role')) = 'management')
  with check ((select (auth.jwt()->>'user_role')) = 'management');

drop policy if exists "Only management can delete par levels" on public.par_levels;
create policy "Only management can delete par levels"
  on public.par_levels for delete
  using ((select (auth.jwt()->>'user_role')) = 'management');

-- RLS Policies for inventory_logs table
drop policy if exists "Everyone can view inventory logs" on public.inventory_logs;
create policy "Everyone can view inventory logs"
  on public.inventory_logs for select
  using (true);

drop policy if exists "Anyone can create inventory logs" on public.inventory_logs;
create policy "Anyone can create inventory logs"
  on public.inventory_logs for insert
  with check (true);

-- Staff can only update their own same-day logs
drop policy if exists "Staff can update own same-day logs" on public.inventory_logs;
...
drop policy if exists "Management can update any log" on public.inventory_logs;
